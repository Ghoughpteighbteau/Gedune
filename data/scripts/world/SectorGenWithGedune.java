...
public class SectorGenWithGedune implements SectorGeneratorPlugin
{
	public void generate( SectorAPI sector )
	{
      ...
      SectorEntityToken GDstation = system.addOrbitalStation(planet,45, 300, 50,"Refactory", "gedune");
      ...
      GDcargo.addCrew(CargoAPI.CrewXPLevel.GREEN,(int)(Math.random()*300)+200);
      GDcargo.addCrew(CargoAPI.CrewXPLevel.REGULAR,(int)(Math.random()*120)+60);
      GDcargo.addCrew(CargoAPI.CrewXPLevel.VETERAN,(int)(Math.random()*40)+20);
      ...
      String[] ships = new String[] {"gedune_chua_wing", "gedune_chua_wing", "gedune_duri_wing", "gedune_duri_wing", "gedune_duri_wing"};
      for(int i = 0; i < ships.length; i++)
         GDmoths.addFleetMember(fac.createFleetMember(FleetMemberType.FIGHTER_WING, ships[i]));
      
      ships = new String[]{"gedune_tenzen", "gedune_nanda", "gedune_nanda", "gedune_nanda", "gedune_mozok", "gedune_mozok_t", "gedune_mozok_f", "gedune_mozok_f", "gedune_kitsune", "gedune_kitsune"};
      for(int i = 0; i < ships.length; i++)
         GDmoths.addFleetMember(fac.createFleetMember(FleetMemberType.SHIP, ships[i] + "_Hull"));
         
		FactionAPI gedune = sector.getFaction( "gedune" );
      //Gedune national policy: "EVERYTHING MUST GO!"
      String[] peopleIhate = new String[] { "hegemony", "tritachyon", "pirates", "pirates", "independent", "player" };
      ...
      String[] peopleIhate2HATEHARDER = {"interstellarFederation", "junk_pirates", "lotus_pirates", "nomads", "serenitystation", "antediluvian", "ghost", "neutrinocorp", "syndicate_asp", "shadow_industry"}; //This is funnier.
